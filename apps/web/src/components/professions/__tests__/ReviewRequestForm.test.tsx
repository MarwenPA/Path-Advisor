/**
 * Tests for ReviewRequestForm — Story 3.7 AC9 frontend.
 *
...
function renderForm(props: Partial<React.ComponentProps<typeof ReviewRequestForm>> = {}) {
  return render(
    <ReviewRequestForm
...
describe("ReviewRequestForm", () => {
  ...
  it("renders reason label", () => {
    renderForm();
    const labels = screen.getAllByText(/raison/i);
    expect(labels.length).toBeGreaterThan(0);
  });

  it("does not call onSubmit when reason is not selected", () => {
    renderForm();
    fireEvent.click(screen.getByRole("button", { name: /envoyer la demande/i }));
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });

  it("shows validation error when submitted without reason", () => {
    renderForm();
    fireEvent.click(screen.getByRole("button", { name: /envoyer la demande/i }));
  ...
  it("updates comment counter as user types", () => {
    renderForm();
    const textarea = screen.getByPlaceholderText(/explique pourquoi/i);
