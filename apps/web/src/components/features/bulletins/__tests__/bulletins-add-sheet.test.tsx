...
describe("BulletinsAddSheet", () => {
  it("does not render sheet content when closed", () => {
    wrap(<BulletinsAddSheet open={false} onClose={vi.fn()} onSuccess={vi.fn()} />);
    expect(screen.queryByRole("dialog")).toBeNull();
  });

  it("renders sheet with title when open", () => {
    wrap(<BulletinsAddSheet open={true} onClose={vi.fn()} onSuccess={vi.fn()} />);
  ...
  it("shows two action buttons: scanner and saisir", () => {
    wrap(<BulletinsAddSheet open={true} onClose={vi.fn()} onSuccess={vi.fn()} />);
    expect(screen.getByRole("button", { name: /scanner.*importer/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /saisir.*main/i })).toBeInTheDocument();
  });

  it("calls onClose when Annuler is clicked", () => {
    const onClose = vi.fn();
    wrap(<BulletinsAddSheet open={true} onClose={onClose} onSuccess={vi.fn()} />);
