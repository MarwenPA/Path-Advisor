  ...
  return (
    <article className="flex flex-col gap-3 rounded-lg border border-border bg-bg p-4">
      <header className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-sm text-text">
            Demande du {formatDateTime(export_.requested_at)}
          </p>
      ...
      {isActive && (
        <p
          aria-live="polite"
          className="rounded bg-bg-2 px-3 py-2 text-sm text-text"
        >
          Préparation de ton export en cours. Cela peut prendre jusqu&apos;à
          30 minutes — tu recevras un email dès qu&apos;il sera prêt.
        </p>
      )}
      ...
      {export_.status === "ready" && (
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-text-subtle">
            L&apos;archive est protégée par un mot de passe. Tu l&apos;as reçu
            dans l&apos;email{" "}
      ...
      {export_.status === "failed" && (
        <div className="flex flex-col gap-2">
          <p className="text-sm text-danger">
            ...
            {export_.error_code ? ` (code ${export_.error_code})` : ""}. Cette
            tentative ne consomme pas ton quota — tu peux retenter immédiatement.
          </p>
          {onRetry && (
