"""Story 8.1 — AC3 outbox contract for the family email module.
...
parent-invitation flow: an SMTP outage leaves a durable, queryable
...
from apps.family.models import ParentInvitation
from apps.family.services.emails import send_invitation_to_parent
...
def test_smtp_outage_leaves_nonsilent_outbox_row(django_capture_on_commit_callbacks):
    ...
    with (
        ...
        assert send_invitation_to_parent(invitation) is True

    row = EmailOutbox.objects.get()
    assert row.to == "parent@test.local"
    assert row.template_app == "family"
    assert row.template_base == "parent_invitation"
