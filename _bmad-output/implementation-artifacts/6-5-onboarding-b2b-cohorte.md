...
| 2026-09-03/04 | dev (claude-sonnet-5) | Revue adversariale + corrections (déjà documentées dans les sessions précédentes, jamais formalisées en §10 avant cette clôture) : `bypass_rls()` ajouté aux 4 endpoints publics d'acceptation (100 % non fonctionnels contre un vrai Postgres RLS sans ça), `with_system_actor()` dans les tâches Celery d'email + gestion `IntegrityError`/finalisation de job, atomicité de `import_row`, appel manquant à `send_request_to_parent()` restauré, migration `0003` créée pour protéger par RLS `counselor_invitations`/`student_import_invitations`/`cohort_import_jobs` (non protégées jusque-là), 4 fichiers de test convertis de `force_authenticate()` à `force_login()`+`bypass_rls()` (le premier ne propage pas jusqu'à `TenantSessionMiddleware`), fixtures `_make_cohort()` corrigées pour combiner `request_context.set_actor()` (thread-local Python, requis par `TenantScopedModel.save()`) et `with_system_actor()` (GUC Postgres, requis par RLS) — deux mécanismes indépendants, tous deux nécessaires. Gap Postgres du 09-02 refermé : 37/37 tests verts sur Postgres réel. |
| 2026-09-05 | dev (claude-sonnet-5) | **Clôture finale** (session live) : vérifié 37/39 → 39/39 tests sur Postgres réel après ajout de 2 tests de régression, `ruff`/`assert_rbac_declared`(243)/`makemigrations --check` clean. **Bug bloquant trouvé et corrigé** : les 2 pages frontend (`invitation-conseillere`, `invitation-eleve` — jamais passées par `npm test`/`tsc` avant cette clôture, gap honnête documenté ci-dessus) faisaient un `fetch()` brut lisant `process.env.API_BASE_URL`, une variable **jamais définie** nulle part (docker-compose définit `API_URL_SERVER`) — repli silencieux sur `http://localhost:8000`, injoignable depuis le conteneur `web` en Docker ; chaque visite affichait "Ce lien n'est plus valide" même pour une invitation valide. Corrigé en routant par `fetchCounselorInvitationStatus`/`fetchStudentInvitationStatus` (déjà correctement écrites via `apiFetch`, jamais utilisées par les pages elles-mêmes). Vérifié en live sur Docker avec une vraie invitation créée à la volée : formulaire affiché correctement. **3 findings majeurs de la revue initiale corrigés** : (1) accepter une invitation conseillère avec un email déjà enregistré levait une `IntegrityError` non gérée → 500 ; ajouté `CounselorEmailAlreadyRegistered` (409) + vérification anticipée + catch de la race TOCTOU. (2) `Cohort` n'avait aucune contrainte d'unicité `(establishment, name, school_year)` — migration `0004` + `CohortAlreadyExists` (409). (3) 13 nouveaux tests frontend (2 pages + 2 formulaires), le gap "Frontend non passé par npm test" du 09-02 est refermé. **Findings restants, documentés dans `deferred-work.md`** (volume trop important pour cette passe) : normalisation casse UAI, validation en-tête/délimiteur CSV, vérification taille fichier avant lecture, check licence établissement à l'accept conseillère, throttle/audit tentatives token invalide, `validate_password(user=...)`. Vérifié : 1319 passed backend (0 régression) ; 785 passed frontend (12 échecs pré-existants sans rapport) ; smoke tests Docker bout en bout. Status → `done`. |
