...
- [ ] [Review][Decision] AC2's tap-through to a métier/parcours detail view is entirely unimplemented — `ParentDashboard`/`parent-dashboard.tsx` renders `ScoreVocationnel` cards and "mes paris" rows with zero `href`/`onClick`/`router.push`; tapping a card does nothing. No parent-scoped detail route exists either. Needs a decision: wire cards to the existing `/metiers/[slug]` and `/schools/[slug]` pages (requires confirming those routes' RBAC allows the `parent` role and still hide bulletin-adjacent sections), build dedicated parent-scoped detail views, or formally narrow AC2 to a future story. [apps/web/src/components/features/parent/parent-dashboard.tsx]
- [ ] [Review][Patch] Parent-visible `first_name` is the **unmasked** raw email local-part (`student.email.split("@")[0]`) on both new parent endpoints — this is the exact leak pattern this same diff's own review-fix pass already closed on `parent_invitation_status` via `mask_local_part()`. Apply the same helper here. [apps/api/apps/family/services/parent_view.py `_child_summary`, apps/api/apps/family/views.py `parent_children_collection`]
- [ ] [Review][Patch] A soft-deleted or suspended student (`status != ACTIVE`, or within the 30-day deletion grace window) is still fully served by the parent dashboard — `resolve_linked_child`/`get_linked_children` check only `ParentStudentLink.revoked_at__isnull=True`, never `student.status`/`is_active`/`deleted_at`. Add an active-status check consistent with how other endpoints treat a soft-deleted user. [apps/api/apps/family/services/parent_view.py `resolve_linked_child`, `get_linked_children`]
- [ ] [Review][Patch] "Métiers explorés" actually renders `compute_recommendations`' AI-scored top results, not a record of métiers the student actually opened/explored — an undisclosed formal deviation from the epic's "métiers que mon enfant a explorés" framing (only informally noted in Completion Notes). Add an explicit documented-deviation note here in §5/Dev Notes, matching how the FavoriteSchool/ParcoursCard swap is disclosed. [apps/api/apps/family/services/parent_view.py `get_child_professions`]
- [ ] [Review][Patch] No `[:8]` slice on the professions list despite Dev Notes/Completion Notes claiming "maps the top-8" — add the limit or correct the documentation to match actual (unbounded) behavior. [apps/api/apps/family/services/parent_view.py `get_child_professions`]
- [x] [Review][Defer] AI-service free-text fields (`signals_contributifs`, `phrase_recopiable`) are passed through to the parent dashboard with no content filter against bulletin-derived phrasing (`compute_recommendations` sends a `bulletin_summary` to the ai-service; nothing prevents it echoing bulletin-adjacent text back in a signal label). Pre-existing characteristic of the Epic 3 recommendation engine's signal design, cross-cutting beyond 6.2's scope — deferred pending an audit of `compute_recommendations`' signal content if bulletin-phrase leakage is ever observed. [apps/api/apps/family/services/parent_view.py `get_child_professions`, apps/api/apps/recommendations/services/recommendation_service.py]
- [x] [Review][Defer] `ParentStudentLink.tenant_id` is written but never read/enforced — `resolve_linked_child`/`get_linked_children` never cross-check parent vs. child `tenant_id`. No impact today (current parent-child links are B2C, `tenant_id=None`); relevant only if a future B2B story introduces tenant-scoped parent access. [apps/api/apps/family/services/parent_view.py, apps/api/apps/family/models.py]

**Dismissed as noise:** extra `child` field in the dashboard payload vs. AC1's literal "uniquement" wording (benign summary data, not the leak intent the AC was guarding against); TOCTOU between the `mes_paris` and `couts_estimes` reads (Low, cosmetic inconsistency only, per the Blind Hunter's own assessment); removal of the `postgresql_only` marker from some pre-existing family tests (intentional consequence of the same diff's legitimate marker-semantics fix, not a coverage regression — the authorization logic itself is DB-backend-agnostic).

**Corroborated clean:** the Blind Hunter layer found **no surviving High/Medium authorization-bypass or information-leakage finding** for the parent-dashboard access-control path itself — `resolve_linked_child` is confirmed as the sole, correctly-ordered authorization gate, and the 6.1 email-mismatch/password fixes in the same diff correctly close the bypasses they targeted.
