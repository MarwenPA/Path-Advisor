...
- 2026-09-05 — Cascade de bugs trouvés et corrigés en session live sur `/profile` (page jamais réellement exercée en conditions réelles jusque-là) :
  1. **Bloqué sur "Chargement…" à l'infini** — `use-student-profile.ts` faisait un `fetch("/api/v1/...")` brut avec un chemin relatif, résolu contre le serveur Next lui-même plutôt que le backend Django (404 systématique) ; la page ne teste que `isLoading`, jamais l'état d'erreur. Corrigé en passant par `apiFetch` (le wrapper standard du repo), comme tout le reste de `lib/api/*`.
  2. **Crash `nextActions is not iterable`** une fois le bug 1 corrigé — `use-maturity-level.ts` avait le même bug de fetch relatif (donc `maturity` restait toujours `undefined`), et `<ProfileMaturityIndicator />` était appelé sans ses props obligatoires (`level`/`nextActions`), qui itèrent `nextActions` sans garde. Corrigé : hook réparé + rebranché sur `useMaturityLevel`, comme `ProgressionModule`.
  3. **`/profile` orphelin de toute navigation** — vivait dans le groupe de routes `(auth)` sans aucun guard ni sidebar/nav (Story 1.15). Déplacé vers `(authenticated)/profile/`, ajouté à `ROUTE_ALLOWED_ROLES` et `NAV_ITEMS` (5ᵉ item élève) — accessible depuis `/accueil` et retour possible.
  4. **Les 3 panneaux d'édition (passions/niveau/bulletins) ne sauvegardaient rien** — stubs explicitement documentés comme tels dans leurs propres commentaires ("Story 2.1/2.2/2.3 wired here in production"). Rebranchés sur les vrais composants et endpoints de l'onboarding (voir Stories 2.1/2.2 Change Log) ; le panneau bulletins renvoie vers `/onboarding/step-3` plutôt que de dupliquer la machine à états OCR/upload dans un `Sheet`.
  5. `profile/history/page.tsx` — même bug de fetch relatif que le point 1, corrigé proactivement (même famille de fichiers).
