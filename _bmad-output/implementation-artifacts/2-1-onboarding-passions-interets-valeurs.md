...
- 2026-09-05 — Bug post-implémentation trouvé et corrigé en session live : `/onboarding` (page d'index) était resté figé sur le placeholder statique de la Story 1.3 ("l'onboarding arrive avec Story 2.1... reviens vite !"), jamais mis à jour une fois le vrai flow (step-1/2/3) livré. Rien ne redirigeait plus loin — tout utilisateur atterrissant sur `/onboarding` (retour après vérification email, navigation directe) restait bloqué sur ce message permanent. Fix : `redirect("/onboarding/step-1")`, qui gère déjà lui-même le cas "étape déjà complétée".
