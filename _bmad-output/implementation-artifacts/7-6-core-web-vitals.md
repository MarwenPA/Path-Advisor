...
✅ `lighthouserc.json` : `categories:performance ≥ 0.80`, `categories:seo ≥ 0.95`, `largest-contentful-paint ≤ 3000ms` (budget CI calibré pour le runner partagé, cible produit réelle 2500ms), `cumulative-layout-shift ≤ 0.1`, `total-blocking-time ≤ 200ms` (proxy moderne de FID — Lighthouse ne mesure plus FID directement). `numberOfRuns: 3` (médiane).
