...
- 2026-09-05 — Bug bloquant trouvé et corrigé en session live : `upload-progress.tsx` importait `Progress` depuis `@/components/ui/progress` (jamais créé) et `bulletin-recap-editor.tsx` importait `Tabs`/`TabsList`/`TabsTrigger`/`TabsContent` depuis `@/components/ui/tabs` (jamais créé, `@radix-ui/react-tabs` jamais installé) — `/onboarding/step-3` plantait avec `Module not found` dès "Continuer avec les bulletins", deux fois de suite (un fix révélait le suivant). Ajouté `components/ui/progress.tsx` (barre déterminée maison, pas de dépendance Radix — même logique que le popover compte de la Story 1.15) et `components/ui/tabs.tsx` (vrai wrapper `@radix-ui/react-tabs`, cohérent avec `dialog`/`select`/`checkbox`/`radio-group` déjà Radix dans ce repo) + la dépendance ajoutée à `package.json`. Bonus : garde `typeof window.matchMedia === "function"` ajoutée dans `upload-progress.tsx` (jsdom, l'environnement de test, n'a pas `matchMedia` du tout — faisait planter tous les tests du fichier).
