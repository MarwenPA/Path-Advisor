...
- 2026-09-06 — Touchée par la Story 7.7 (i18n) : les 4 composants de la page et `app/page.tsx` migrés vers `next-intl` (`messages/fr.json#homepage.*`). **Écart mineur vs. AC4** : le `export const metadata` statique est devenu un `generateMetadata()` async, `getTranslations` étant asynchrone. L'exigence de fond est inchangée et vérifiée en live — `/` expose toujours un `title` + `meta description` dédiés (pas ceux du layout racine). Aucune URL ni contenu modifié ; ACs 1/2/3/5 inchangés.
